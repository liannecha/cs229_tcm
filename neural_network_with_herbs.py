...
with torch.no_grad():
    for i in range(0, len(first_dataset), 25):
        ...
        pred_concept_vec = pred_concepts.squeeze(0).numpy()
        pred_syndrome_idx = torch.argmax(pred_syndromes, dim=1).item()
        ...
        # get true herbs for this syndrome from the prior
        true_herb_set = set(np.array(herb_ids)[syndrome_herb_prior[true_syndrome_idx.item()] > 0])
